...
#20.04: I added two zeros to the variance_epsilon for batch_norm
...
def generator(input, g_weights):
 ...
 #   x = tf.nn.leaky_relu(x) 
    ...
    x = conv3d_no_act(x, g_weights[15], padding="SAME") 
    return x
...
def combined(input, g_weights, d_weights):
    ...
    x = conv3d(x, g_weights[12], padding="VALID") 
    x = batch_norm(x,g_weights[13],g_weights[14])
    x = conv3d_no_act(x, g_weights[15], padding="VALID") 
...
def disc_loss(discriminator_true, energy_batch, ecal_batch, g_weights, d_weights, batch_size, latent_size=200):  #energy = aux
    ...
    #aux loss
    loss_aux_true = tf.reduce_mean(tf.reduce_mean(tf.math.abs((energy_batch - discriminator_true[1])/(energy_batch + 2e-7))) *100)
    loss_aux_fake = tf.reduce_mean(tf.reduce_mean(tf.math.abs((gen_aux - discriminator_fake[1])/(gen_aux + 2e-7))) *100)
    loss_aux = (loss_aux_true + loss_aux_fake)/2.
    
    #ecal loss
    loss_ecal_true = tf.reduce_mean(tf.reduce_mean(tf.math.abs((ecal_batch - discriminator_true[2])/(ecal_batch + 2e-7))) *100)
    loss_ecal_fake = tf.reduce_mean(tf.reduce_mean(tf.math.abs((gen_ecal - discriminator_fake[2])/(gen_ecal + 2e-7))) *100)
