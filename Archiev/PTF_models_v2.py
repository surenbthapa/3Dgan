...
#v2: selfmade loss functions, losses in loss table are now without weights, as in the keras model
...
def batch_norm(x, off, scale): #a conv layer with 16 channels has 32 learnable parameters and 32 nonlearnable
    ...
    x = tf.nn.batch_normalization(x, mean, variance, offset=off, scale=scale, variance_epsilon=0.0001, name=None)#the more sparse the images are, the higher can the variance_epsilon value be
    return x
...
#@tf.function
def generator(input, g_weights):
    x = tf.reshape(input, shape=[-1,200])
    x = dense(x, g_weights[0])
    x = tf.nn.leaky_relu(x) 
    x = tf.reshape(x, shape=[-1,5,5,5,5])    #num, ch, x,w,h
    x = conv3d(x, g_weights[1], padding="SAME") 
    x = batch_norm(x,g_weights[2],g_weights[3])
    x = conv3d_transpose(x, g_weights[4], stride=3)
    x = conv3d(x, g_weights[5], padding="SAME") 
    x = batch_norm(x,g_weights[6],g_weights[7])
    x = conv3d_transpose(x, g_weights[8], stride=2)
    x = conv3d(x, g_weights[9], padding="VALID") 
    x = batch_norm(x,g_weights[10],g_weights[11])
    x = conv3d(x, g_weights[12], padding="VALID") 
    x = batch_norm(x,g_weights[13],g_weights[14])
    x = conv3d(x, g_weights[15], padding="VALID") 
#    x = tf.nn.relu(x)
    return x
...
#@tf.function
def discriminator(input, g_weights, d_weights):
    x = input
    x = conv3d(x, d_weights[0], dropout=0.2)
    x = padding(x,2,2,2)
    x = conv3d(x, d_weights[1], padding="VALID", dropout=0.2)
    x = batch_norm(x,d_weights[2],d_weights[3])
    x = padding(x,2,2,2)
    x = conv3d(x, d_weights[4], padding="VALID", dropout=0.2)
    x = batch_norm(x,d_weights[5],d_weights[6])
    x = padding(x,1,1,1)
    x = conv3d(x, d_weights[7], padding="VALID", dropout=0.2)
    x = batch_norm(x,d_weights[8],d_weights[9])
    x = tf.nn.avg_pool3d(x, ksize=2, strides=2, padding="VALID", data_format='NCDHW')
    
    x = tf.reshape(x, shape=[tf.shape(x)[0], -1])  # flatten
#    print("flatten: ", x.shape)

    fake = dense(x, d_weights[10])
    fake = tf.nn.sigmoid(fake)
#    print("\nfake: ", fake.shape)
    
    aux = dense(x, d_weights[11])   #this is the identity function (slope one)
#    print("aux: ", aux.shape)
    
    ecal = tf.math.reduce_sum(input, axis=(2,3,4))
#    print("ecal: ", ecal)
...
#@tf.function
def combined(input, g_weights, d_weights):
    x = tf.reshape(input, shape=[-1,200])
    x = dense(x, g_weights[0])
    x = tf.nn.leaky_relu(x) 
    x = tf.reshape(x, shape=[-1,5,5,5,5])    #num, ch, x,w,h
...
#    x = tf.nn.relu(x)
    ...
    fake = tf.nn.sigmoid(fake)
    aux = dense(x, d_weights[11])   #this is the identity function (slope one)
    ecal = tf.math.reduce_sum(input_disc, axis=(2,3,4))
    return fake, aux, ecal


def bit_flip_tf(x, prob=0.1):
...
def disc_loss(discriminator_true, energy_batch, ecal_batch, g_weights, d_weights, batch_size, latent_size=200):  #energy = aux
    ...
    discriminator_fake = discriminator(generated_images, g_weights, d_weights)
    ...
    loss_true = tf.reduce_mean(- label_true * tf.math.log(discriminator_true[0] + 1e-7) - 
                               (1 - label_true) *tf.math.log(1 - discriminator_true[0] + 1e-7))
    loss_fake = tf.reduce_mean(- label_fake * tf.math.log(discriminator_fake[0] + 1e-7) - 
                               (1 - label_fake) *tf.math.log(1 - discriminator_fake[0] + 1e-7))
    loss_true_fake = (loss_true + loss_fake)/2.


    #aux loss
    loss_aux_true = tf.reduce_mean(tf.reduce_mean(tf.math.abs((energy_batch - discriminator_true[1])/(energy_batch + 1e-7))) *100)
    loss_aux_fake = tf.reduce_mean(tf.reduce_mean(tf.math.abs((gen_aux - discriminator_fake[1])/(gen_aux + 1e-7))) *100)
    loss_aux = (loss_aux_true + loss_aux_fake)/2.
    
    #ecal loss
    loss_ecal_true = tf.reduce_mean(tf.reduce_mean(tf.math.abs((ecal_batch - discriminator_true[2])/(ecal_batch + 1e-7))) *100)
    loss_ecal_fake = tf.reduce_mean(tf.reduce_mean(tf.math.abs((gen_ecal - discriminator_fake[2])/(gen_ecal + 1e-7))) *100)
...
def gen_loss(g_weights, d_weights, batch_size, latent_size=200):
    noise, gen_aux, generator_input, gen_ecal = func_for_gen_tf(nb_test=batch_size, latent_size=latent_size) #len has to be batch size, because of bit_flip used below
    discriminator_fake = combined(generator_input, g_weights, d_weights)
